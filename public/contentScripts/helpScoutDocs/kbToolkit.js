const KB_TEST_ARTICLE_URL = 'https://secure.helpscout.net/docs/5c2ba7d504286304a71dc91f/article/'
...
const checkKbArticle = async () => {
  ...
  if (
    (url.includes(KB_TEST_ARTICLE_URL) || url.includes(KB_PUBLIC_ARTICLE_URL)) &&
    ...
    document.getElementById('kbTogglerButton').addEventListener('click', toggleKbToolkit)
    ...
    document.getElementById('kb-ul').addEventListener('click', injectKbUnorderedList)
    document.getElementById('kb-device').addEventListener('click', injectKbDeviceList)
    document.getElementById('kb-table').addEventListener('click', injectKbTable)
    document.getElementById('kb-spanish').addEventListener('click', injectKbSpanishLink)

    //description list
    document.getElementById('kb-dl').addEventListener('click', injectKbDescriptionList)
    document.getElementById('kb-ds').addEventListener('click', injectKbDescriptionStep)

    //media
    document.getElementById('kb-img').addEventListener('click', injectKbImage)
    document.getElementById('kb-loom').addEventListener('click', injectKbLoomVideo)
    document.getElementById('kb-youtube').addEventListener('click', injectKbYouTube)
    document.getElementById('kb-class-mobile').addEventListener('click', injectKbClassMobile)
    ...
    //callouts
    document.getElementById('kb-cy').addEventListener('click', injectKbCalloutYellow)
    document.getElementById('kb-cr').addEventListener('click', injectKbCalloutRed)
    document.getElementById('kb-cg').addEventListener('click', injectKbCalloutGreen)
    document.getElementById('kb-cb').addEventListener('click', injectKbCalloutBlue)
    document.getElementById('kb-12-word').addEventListener('click', injectKbCallout12Words)
    document.getElementById('kb-pr-keys').addEventListener('click', injectKbCalloutPrKeys)
    ...
    document.getElementById('kb-bi').addEventListener('click', injectKbBoldAndItalic)
